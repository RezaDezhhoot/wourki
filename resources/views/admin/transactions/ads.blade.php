                    ...
                    <div class="col-sm-12">
                        <div class="card-box">
                            ...
                            <form role="form" action="{{ URL::current() }}" method="get">
                                ...
                                <div class="col-md-3">
                                    <div class="form-group">
                                            <label for="type">لینک به :</label>
                                            <select name="type" id="type"
                                                    class="js-example-basic-single">
                                                    <option value="store" {{(app('request')->type == "store" || !app('request')->type) ? 'selected' : ''}}>فروشگاه</option>
                                                    <option value="product" {{app('request')->type == "product"  ? 'selected' : ''}}>محصول / خدمت</option>
                                            </select>
                                        </div>
                                    </div>
                                    <div class="col-md-3">
                                    <div class="form-group">
                                            ...
                                            <select name="show" id="show"
                                                    class="js-example-basic-single">
                                                    <option value="all" {{(app('request')->show == "all" || !app('request')->show) ? 'selected' : ''}}>همه</option>
                                                    <option value="self" {{app('request')->show == "self"  ? 'selected' : ''}}>تبلیغات اولیه</option>
                                                    <option value="stairs" {{app('request')->show == "stairs"  ? 'selected' : ''}}>ارتقا های تبلیغات</option>
                                            </select>
                                        </div>
                                    </div>
                                </div>
                                        ...
                                        <table class="table">
                                            ...
                                            @foreach($ads as $row)
                                                <tr>
                                                    ...
                                                    <td>{{ $row->pay_status == "paid" ? 'پرداخت شده' : 'پرداخت نشده' }}</td>
                                                    <td>{{ $row->payment_type == "online" ? 'آنلاین' : 'کیف پول' }}</td>
                                                    <td>{{ $row->initial_pay == "stairs" ? 'ارتقا تبلیغ' : 'ایجاد تبلیغ' }}</td>
                                                    <td>{{ \Morilog\Jalali\Jalalian::forge($row->updated_date)->format('Y/m/d H:i:s') }}</td>
                                                </tr>
                                            @endforeach
                                            </tbody>
                                        </table>

        ...
        <div class="content">
            <div class="container">
                <div class="row">
                    <div class="col-sm-12">
                        <div class="card-box">
                            <h4 class="m-t-0 header-title"><b>فیلتر کردن نظرات</b></h4><br>
                            <div class="row">
                                @if($errors->any())
                            ...
                            <form role="form" action="{{ URL::current() }}" method="get">
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-3">
                                        <div class="form-group">
                                            ...
                                            <input type="text" value="{{ request()->input('user_full_name') }}" name="user_full_name" id="user_full_name" class="form-control">
                                        </div>
                                    </div>
                                    <div class="col-md-3">
                                        <div class="form-group">
                                            <label for="user_mobile">تلفن همراه کاربر</label>
                                            <input type="text" value="{{ request()->input('user_mobile') }}" name="user_mobile" id="user_mobile" class="form-control">
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="row">
                                    <div class="col-md-12">
                                        <button type="submit" id="send" class="btn btn-sm btn-purple waves-effect waves-light">اعمال</button>
                                        <a href="{{ url()->current() }}" class="btn btn-sm btn-default waves-effect waves-light">حذف فیلترها</a>
                                    </div>
                                </div>
                            </form>
                        </div>
                                                                ...
                                                                <div class="btn-group m-b-20">
                                                                    <div class="btn-group">
                                                                        ...
                                                                        <ul class="dropdown-menu parent">
                                                                            <li>
                                                                                <button class="btn btn-success btn-block btn-xs approved-store-status"
                                                                                        value="{{ $comment->comment_id }}">
                                                                                    تاییده شده
                                                                                </button>
                                                                            </li>
                                                                            <li>
                                                                                <button class="btn btn-warning btn-block btn-xs reject-store-status"
                                                                                        value="{{ $comment->comment_id }}">رد
                                                                                    کردن
                                                                                </button>
                                                                            </li>
                                                                            <li>
                                                                                <button class="btn btn-info btn-block btn-xs pending-store-status"
                                                                                        value="{{ $comment->comment_id }}">
                                                                                    درانتظارتایید
                                                                                </button>
                                                                            </li>
    ...
    <script>
        $(document).ready(function () {
            ...
            $('.approved-store-status').click(function (e) {
                e.preventDefault();
                var $this = $(this).closest("tr").attr('data-id');
                var dataStatusButton = $(this).closest("tr").find($('[data-status-button]'));
                $.ajax({
                    type: 'GET',
                    ...
                    success: function () {
                        dataStatusButton.removeClass().addClass('btn btn-xs btn-success dropdown-toggle waves-effect').html('تاییده شده <span class="caret"></span>');
                    }
                });
            });
            ...
            $('.reject-store-status').click(function (e) {
                ...
                $.ajax({
                    ...
                    success : function () {
                        dataStatusButton.removeClass().addClass('btn btn-xs btn-warning dropdown-toggle waves-effect').html('رد شده <span class="caret"></span>');
                    }
                });
            });
            ...
            $('.pending-store-status').click(function (e) {
                ...
                $.ajax({
                    ...
                    success : function () {
                        dataStatusButton.removeClass().addClass('btn btn-xs btn-info dropdown-toggle waves-effect').html('در انتظار تایید <span class="caret"></span>');
                    }
                });
            });

            $('#send').click(function (e) {
                if ( $('#store_name').val() == null && $('#product_name').val() == null && $('#dateRangePicker').val() == '' && $('#status').val() == null && $('#user_mobile').val() == null && $('#user_full_name').val() == null) {
                    e.preventDefault();

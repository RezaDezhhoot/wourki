...
class DiscountController extends Controller
{
    ...
    public function validateDiscount(Request $request){
        ...
        if($request->type == 'product-service'){
            ...
            foreach ($carts as $index => $cart) {
                ...
                $discount = Discount::getDiscountFor($request->code , $carts[$index]->product->store->store_type , $carts[$index]->product->id);
                if(is_null($discount))
                $sumPrice += $carts[$index]->totalPrice;
                else
                $sumPrice += $discount->applyOn($carts[$index]->totalPrice);
            }
            }
            else{
        ...
        if($request->price && ($discount->min_price >= $request->price || $discount->max_price <= $request->price)){
            return response()->json(['errors', ['تخفیف مورد نظر یافت نشد']], 404);
        }
    ...
    public function createUserStoreDiscount(Request $request){
        $request->validate([
            ...
            'name.required' => 'لطفا نام تخفیف را وارد نمایید',

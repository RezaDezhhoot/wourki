...
class SaveBillFacade
{
    ...
    public function save()
    {
        ...
        if ($this->payType->getPayType() == PaymentType::PAYMENT_TYPE_WALLET) {
            ...
            $billHandler = new BillHandler($this->user, $this->address, PaymentType::PAYMENT_TYPE_WALLET, $this->discount);
            $billHandler->save();
            if (count($cartDecorator->usedDiscounts) > 0) {
            UsedDiscount::whereIn('id' , $cartDecorator->usedDiscounts)->update([
                'status' => 'approved',
                'pay_type' => 'wallet'
            ]);
        }
            ...
            // the address id is stored in session to be used after returning from payment gateway
            request()->session()->put('address', $this->address->address_id);
            $onlinePayment = new Payment(new Online());
            $onlinePayment->init([
                'cost' => $totalCartPrice * 10,
                'callback_url' => route('verify.bill'),
            ...
            session()->put('used_discount_ids',json_encode($cartDecorator->usedDiscounts));
        }
            return $onlinePayment->pay();
        }
